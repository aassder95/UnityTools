...
Unity 클라이언트 프로젝트에서 Claude가 따라야 할 공용 작업 지침이다.

## 1. 작업 정책

### 도구 실행

- 코드 작업 전에 현재 repo와 변경 대상 하위 폴더의 `CLAUDE.md`를 읽고 처음부터 코드 작성 기준으로 적용한다.
- repo 읽기/검색, `CLAUDE.md` 확인, 코드 읽기처럼 필요한 작업이 permission 거부로 실패하면 같은 명령을 표현만 바꿔 반복 시도하지 않는다.
- 권한이 필요한 도구 호출은 사용자 승인을 기다리고, 거부되면 이유를 확인한 뒤 실제 repo와 사용자 제공 맥락을 기준으로 진행한다.
- 권한 거부 과정은 중간 보고에 반복 나열하지 않는다. 필요하면 최종 응답에 CLAUDE.md 확인 여부와 메모리 사용 여부만 짧게 적는다.
...
- stash가 꼭 필요하면 작업 전 목록을 확인하고 Claude가 만든 항목임을 식별할 수 있는 메시지를 사용한다.
- Claude가 만든 stash는 같은 작업 안에서 pop/apply 후 drop한다. 충돌이나 미복구 상태면 삭제하지 말고 이름과 이유를 보고한다.
...
- Claude는 최종 수정이라고 판단해 `[TEST]` 로그를 임의 삭제하지 않는다.
